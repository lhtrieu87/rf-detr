...
class LWDETR(nn.Module):
    ...
    def reinitialize_detection_head(self, num_classes):
        """
        Reinitialize the detection head when the number of classes differs from pre-trained model.
        This handles transfer learning scenarios where the target dataset has different number of classes.

        Args:
            num_classes: Target number of classes for the new task
        """
        # Get the original number of classes from the classification layer
        base = self.class_embed.weight.shape[0]

        # Calculate how many times we need to repeat the weights to cover all new classes
        # This ensures we have at least `num_classes` rows in our weight matrix
        num_repeats = int(math.ceil(num_classes / base))

        # Repeat the classification weights to create initialization for new classes
        # This preserves learned representations from pre-training while expanding capacity
        self.class_embed.weight.data = self.class_embed.weight.data.repeat(
            num_repeats, 1
        )

        # Trim the weights to exactly match the target number of classes
        # Removes any excess rows created by the repeat operation
        self.class_embed.weight.data = self.class_embed.weight.data[:num_classes]

        # Apply the same repeat-and-trim process to classification biases
        ...
        # Handle two-stage detection: encoder also has classification outputs
        # In two-stage DETR, the encoder generates proposals with class predictions
        if self.two_stage:
            # Reinitialize encoder classification layers for each group
            # group_detr enables training acceleration by using multiple query groups
            for enc_out_class_embed in self.transformer.enc_out_class_embed:
                # Repeat encoder classification weights using the same strategy
                enc_out_class_embed.weight.data = (
                    enc_out_class_embed.weight.data.repeat(num_repeats, 1)
                )
                # Trim to target number of classes
                enc_out_class_embed.weight.data = enc_out_class_embed.weight.data[
                    :num_classes
                ]
                # Repeat and trim encoder classification biases
    ...
    def forward(self, samples: NestedTensor, targets=None):
        ...
        if hs is not None:
            ...
            # The same class_embed for all decoder layers
    ...
    def _set_aux_loss(self, outputs_class, outputs_coord, outputs_masks):
        ...
        # [:-1] selects all layers except the last one.
        # The last layer is excluded here because it is treated as the "main" output, not an "auxiliary" output.
        if outputs_masks is not None:
            return [
    ...
    def loss_labels(self, outputs, targets, indices, num_boxes, log=True):
        ...
        if self.ia_bce_loss:
            ...
            iou_targets = torch.diag(
                box_ops.box_iou(
                    # IoU shouldn’t backprop into box regression through this branch.
                    # IoU is used as a fixed target weight.
                    box_ops.box_cxcywh_to_xyxy(src_boxes.detach()),
                    box_ops.box_cxcywh_to_xyxy(target_boxes),
                )[0]
            )
            # Compute IoU between matched predicted boxes and GT (pos_ious).
            pos_ious = iou_targets.clone().detach()
            # Probability of the predicted boxes, non-background.
            # Sigmoid is independent per class.
            ...
            # If alpha ≈ 1: mostly probability-aware (like focal).
            # If alpha ≈ 0: mostly IoU-aware (instance quality).
            # In between: blend of confidence + localization quality.
            # Build a positive weight t = p^α * IoU^(1−α)
            # Mix classification confidence and localization quality.
            t = prob[pos_ind].pow(alpha) * pos_ious.pow(1 - alpha)
            # Avoids t = 0 → avoids completely zeroing gradients.
            t = torch.clamp(t, 0.01).detach()

            # For positive locations:
            #   pos_weights = t
            #   neg_weights = 1 - t
            # If IoU/ prob low → t small
            # → positive term small, negative term large (penalize this positive more).
            # For negative locations:
            #   pos_weights = 0
            #   neg_weights = prob**gamma (unchanged)
            # If the model mistakenly predicts high prob for wrong class
            # → (prob^gamma) is large → big loss
            # Two types of negatives
            # There are two types:
            # 1) Unmatched queries
            # These queries correspond to "no object".
            # All classes are negative.
            # 2) Wrong classes within matched queries
            # For matched query q and true class c*,
            # all other classes c != c* are treated as negatives.
            ...
            # Matched by Hungarian algorithm but IoU is zero, consider as negative samples.
            # Higher logits for boxes that have higher IoU with ground truth.
            src_boxes = outputs["pred_boxes"][idx]
            target_boxes = torch.cat(
                [t["boxes"][i] for t, (_, i) in zip(targets, indices)], dim=0
            )
            ...
            # When IoU = 0, the “positive” class becomes a negative,
            # so the model is punished for giving it confidence and learns to score bad boxes low.

            pos_ind = [id for id in idx]
            pos_ind.append(target_classes_o)
            cls_iou_func_targets[pos_ind] = pos_ious_func
            # This divides by maximum IoU per query across classes.
            # Because each query only has one class with IoU > 0, this becomes:
            #   Positive class → 1
            #   All other classes → 0
            ...
            # Different from use_position_supervised_loss,
            # use IoU as the target score directly instead of 0 or 1 after normalization.
            src_boxes = outputs["pred_boxes"][idx]
            target_boxes = torch.cat(
                [t["boxes"][i] for t, (_, i) in zip(targets, indices)], dim=0
            )
            ...
            # It creates a [B, num_queries] tensor where every entry is filled with self.num_classes,
            # which represents the “no-object” class.
