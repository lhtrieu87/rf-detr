...
class MultiScaleProjector(nn.Module):
    ...
    def forward(self, x):
        ...
        # It creates each scale independently (instead of sequentially like FPN).
        # x list of len(in_channels), each is (B, C, H, W)
        # f2   (B, C, 45, 45)
        # f4   (B, C, 45, 45)
        # f5   (B, C, 45, 45)
        # f9   (B, C, 45, 45)
        ...
        # Scale = 4.0
        # → (B, C_out, 180, 180)   ← highest resolution result
        # Scale = 2.0
        # → (B, C_out, 90, 90)
        # Scale = 1.0
        # → (B, C_out, 45, 45)
        ...
        if self.survival_prob < 1.0 and self.training:
            ...
            for i in range(1, num_features):
                # Later / deeper feature maps have higher probability of being dropped
                # Encourages the model to not depend too heavily on high-level features
