...
class Model:
    def __init__(self, **kwargs):
        ...
        if args.pretrain_weights is not None:
            ...
            if checkpoint_num_classes != args.num_classes + 1:
                # Reinitialize detection head to match target dataset's number of classes
            ...
            # When the pre-trained model has different classes than your target dataset.
            # Exclude query embeddings if using different number of queries
            ...
            # Unlike pretrain_exclude_keys which simply removes parameters,
            # pretrain_keys_modify_to_load transforms the pre-trained weights
            # to better match the target dataset before loading them.
            if args.pretrain_keys_modify_to_load is not None:
                ...
                for modify_key_to_load in args.pretrain_keys_modify_to_load:
                    try:
                        # Maps Object365 classes to COCO classes based on semantic similarity
                        # Selects relevant weights from Object365's 365-class classifier
                        # Reorders/combines them to match COCO's 80-class structure
                        # Returns adapted weights that preserve learned features but match target dimensions
                        # Large dataset → smaller dataset transfers
                        # When datasets have overlapping but different class vocabularies
            ...
            # Handle Group DETR query parameter resizing for resume training
            # We may want to resume training with a different number of groups for group detr
            ...
            for name, state in checkpoint["model"].items():
                if any(name.endswith(x) for x in query_param_names):
                    ...
                    if current_size == num_desired_queries:
                        # Perfect match, no changes needed
                        continue
                    elif current_size > num_desired_queries:
                        # Checkpoint has more queries than needed, truncate
                        checkpoint["model"][name] = state[:num_desired_queries]
                    else:
                        # Checkpoint has fewer queries than needed, expand by repetition
                        # Calculate how many times to repeat to reach desired size
                        repeat_factor = (
                            num_desired_queries + current_size - 1
                        ) // current_size
                        expanded_state = state.repeat(
                            repeat_factor, *([1] * (state.dim() - 1))
                        )
                        # Trim to exact desired size
                        checkpoint["model"][name] = expanded_state[:num_desired_queries]
    ...
    def train(self, callbacks: DefaultDict[str, List[Callable]], **kwargs):
        # Define supported callback hooks for training lifecycle
        # on_fit_epoch_end: Called at end of each epoch with log_stats (metrics, timing, etc.)
        # on_train_batch_start: Called before each training batch forward pass
        # on_train_end: Called once at completion of entire training process
        ...
        # Store reference to the original model before wrapping with DDP
        # This is essential for accessing the actual model parameters and methods
        model_without_ddp = model

        # Configure model for distributed training across multiple GPUs/nodes
        if args.distributed:
            # Synchronize batch normalization across all processes for consistent statistics
            # Standard BatchNorm calculates running mean/variance independently on each GPU
            # SyncBatchNorm ensures all processes use the same batch statistics
            if args.sync_bn:
                # Convert all BatchNorm layers to SyncBatchNorm for distributed training
                # This maintains training consistency but adds communication overhead
                # Critical for small batch sizes per GPU where local statistics would be unreliable
                model = torch.nn.SyncBatchNorm.convert_sync_batchnorm(model)

            # Wrap model with DistributedDataParallel (DDP) for multi-GPU training
            # DDP handles gradient synchronization and parameter updates across processes
            model = torch.nn.parallel.DistributedDataParallel(
                model,
                device_ids=[args.gpu],  # Specify which GPU this process should use
                find_unused_parameters=True,  # Handle parameters that might not receive gradients
                # Set to True for models with conditional execution paths
                # Has performance overhead, set to False if all parameters always receive gradients
            )

            # Store reference to the actual model inside the DDP wrapper
            # DDP wraps the original model in a .module attribute
            # Access via .module is required for:
            # - Model state dict operations (saving/loading checkpoints)
            # - Accessing model-specific methods and attributes
            # - Getting true parameter counts
        ...
        # This enables fine-tuned optimization where different parts of the model learn at different rates
        # Critical for transfer learning and fine-tuning scenarios
        param_dicts = get_param_dict(args, model_without_ddp)

        # Filter out any parameter groups that don't require gradients
        # This handles edge cases where some parameters might be frozen during training
        # Each element in param_dicts is a dict with keys: {"params": tensor, "lr": float, "weight_decay": float}
        param_dicts = [p for p in param_dicts if p["params"].requires_grad]

        # Initialize optimizer with parameter groups having different learning rates
        # AdamW optimizer will apply different learning rates to different model components:
        # - Backbone (encoder): Often lower LR for pre-trained features (args.lr_encoder * decay_factors)
        # - Decoder: Medium LR with component decay (args.lr * lr_component_decay)
        # - Other components: Base learning rate (args.lr)
        # This hierarchical learning rate strategy is essential for effective transfer learning
